...
### KTD3. Внешнего хранилища нет: состояние на клиенте, подборка в ссылке

Ограничение проекта — никаких платных сторонних сервисов. Поэтому Upstash Redis и любые другие управляемые хранилища исключены, а не отложены.

- **Сессия** (реакции, состояние ранжировщика, пул карточек) живёт в браузере и приходит с запросом. Сервер не хранит ничего между запросами.
- **Подборка** кодируется в ссылку параметрами поиска и идентификаторами предложений; страница пересобирает варианты через MCP при открытии. Причина — размер: `checkout_ref` одного транспортного варианта весит 546 байт (замер 2026-08-04), три поездки с жильём дают около 3,4 КБ в кодировке для URL, что режется мессенджерами.
- **Кэша как слоя нет.** Замер показал, что решающий рычаг — прогрев серверного кэша самого Туту (см. KTD10), а не собственный кэш.

**Что это упрощает:** исчезают версионирование сессии, гонки между маршрутами и потеря реакции при попадании запроса в другой процесс — то есть большая часть прежнего юнита U13.

**Что это стоит:** сессия не переносится между устройствами и не переживает очистку хранилища браузера.

### KTD10. Греется кэш Туту, а не строится свой

Замер 2026-08-04: `search_multitransport` Москва → Сочи на холодную — 10,8 с, повторные вызовы — 0,48 и 1,53 с. `search_hotels` — 2,1 с. У MCP Туту серверный кэш, и разница на порядок.

Отсюда: скрипт прогрева проходит по каталогу направлений перед демо и перед хакатоном. Собственный кэш существует только в пределах процесса как best-effort.

**После ревью прогрев понижен в статусе.** Он не гарантируется: срок жизни чужого кэша неизвестен, ключ может включать даты и состав, а на Vercel нет постоянного процесса для прогрева. Поэтому первый экран строится на снапшоте (KTD11), а прогрев лишь ускоряет живые данные, догоняющие снапшот. Его отказ больше не ломает критерий приёмки.

### KTD11. Снапшот направлений в репозитории даёт первый экран

Каталог прогоняется скриптом заранее, результат лежит в репозитории. Первые карточки приходят из него мгновенно, живой поиск идёт параллельно и заменяет их.

Возраст цены показывается на каждой карточке (AC16), замена видна пользователю — снапшот не выдаётся за актуальные данные. Побочная выгода: зафиксированный снимок пула, нужный для воспроизводимости (AC25) и протокола сравнения ранжировщиков, появляется как часть механизма.

### KTD4. Потоковая выдача, контракт которой фиксируется до интерфейса

Ответ на запрос — стрим: сначала карточки снапшота, затем живые по мере готовности направлений.
...
Если даты отеля выводить из выбранного транспортного варианта, цепочка становится последовательной (транспорт → выбор → отель → пакет) и живые данные догоняют снапшот недопустимо долго.
...
flowchart TB
    ...
    Link["Кодирование подборки<br/>в ссылку"]
...
src/lib/
  ...
  session/                     # клиентское состояние сессии, сериализация
  link/                        # кодирование и разбор подборки в ссылке
data/
...
#### U13. Клиентское состояние сессии и прогрев направлений

**Цель:** сессия живёт в браузере и корректно переживает перезагрузку страницы; направления каталога прогреваются скриптом.

**Зависимости:** U1.

**Файлы:** `src/lib/session/state.ts`, `src/lib/session/serialize.ts`, `scripts/warm-destinations.mjs`, `tests/session/*.test.ts`

**Подход:** резко сокращён по сравнению с прежней редакцией — внешнего хранилища нет, поэтому версионирование, распределённый single-flight и защита от гонок между процессами не нужны.

Состояние сессии сериализуется компактно и хранится в браузере, переживая перезагрузку. Размер контролируется: пул карточек хранится идентификаторами, а не целыми предложениями.

Скрипт прогрева обходит каталог направлений и дёргает поиск, разогревая серверный кэш Туту (KTD10). Запускается вручную перед демо; фонового процесса на Vercel для этого нет и не заводится.

**Сценарии тестов:**
- Состояние сериализуется и восстанавливается без потери реакций
- Перезагрузка страницы восстанавливает позицию в ленте и состояние ранжировщика (AC11)
- Повторная реакция на одну карточку засчитывается один раз (AC23)
- Размер сериализованного состояния после 15 реакций не превышает заданный предел
- Повреждённое или несовместимое по версии состояние отбрасывается с чистого листа, а не роняет экран

**Верификация:** `node scripts/warm-destinations.mjs` проходит; после прогрева повторный поиск по направлению каталога укладывается в 2 секунды.
...
#### U15. Снапшот направлений

**Цель:** предзаготовленные карточки по каталогу, отдающиеся мгновенно и заменяемые живыми данными.

**Зависимости:** U1, U3.

**Файлы:** `scripts/build-snapshot.mjs`, `data/snapshot/**`, `src/lib/search/snapshot.ts`, `tests/search/snapshot.test.ts`

**Подход:** скрипт проходит каталог направлений на нескольких типичных окнах дат и составах, складывает результат в репозиторий с датой построения. Приложение читает снапшот как обычный источник карточек и отдаёт их первым событием потока, не дожидаясь MCP.

Снапшот версионируется вместе с кодом: он часть сборки, а не внешние данные. Перед защитой перестраивается свежим прогоном — цены двухнедельной давности выглядят хуже, чем их отсутствие.

**Сценарии тестов:**
- Карточка снапшота несёт дату построения и отдаётся с пометкой возраста (AC16)
- Первое событие потока приходит из снапшота, без обращения к MCP (AC7)
- Живая карточка по тому же направлению заменяет снапшотную (AC7a)
- Направление вне снапшота не ломает поток: идёт живой поиск с признаком нового направления (AC7b)
- Пустой или повреждённый снапшот не роняет приложение: работает живой поиск

**Верификация:** `node scripts/build-snapshot.mjs` собирает снапшот; первая карточка на production приходит не позже 1,5 секунды.
...
- Полная недоступность MCP → событие `unavailable`, отличное от пустого `done` (AC10a)
- Ответ 429 с паузой длиннее дедлайна → повтор не делается, кандидат помечается не ответившим

**Верификация:** замер на production-деплое — первое событие `card` из снапшота не позже 1,5 секунды от отправки запроса (AC7); живые карточки заменяют снапшотные не позже 12 секунд.
...
**Цель:** лента карточек с реакциями, первая карточка из снапшота не позже 1,5 секунды от отправки запроса.
...
**Верификация:** замер на production-деплое с мобильного, холодная функция: от отправки запроса до первой карточки — не более 1,5 секунды по p95 из десяти прогонов (AC7); замена живыми данными видна (AC7a).
...
#### U9. Подборка в ссылке и шеринг

**Цель:** до трёх вариантов по ссылке, открываемой на чужом устройстве без сессии.

**Зависимости:** U7, U13.

**Файлы:** `src/app/list/[payload]/page.tsx`, `src/lib/link/encode.ts`, `src/lib/link/decode.ts`, `tests/link/*.test.ts`

**Подход:** ссылка несёт параметры поиска и идентификаторы выбранных предложений, а не сами предложения — иначе она весит около 3,4 КБ и режется мессенджерами. Страница пересобирает варианты через MCP при открытии, поэтому цены на открытой ссылке свежие.

Подборка доступна с 5-й реакции, меняется до 9-й, замораживается на 10-й. Понятия TTL нет: ссылка живёт, пока живут предложения. Исчезнувшее предложение заменяется актуальным лучшим по тем же параметрам с явной пометкой о замене.

**Сценарии тестов:**
- Недоступна до 5-й реакции (AC30)
- Меняется между 5-й и 9-й реакцией, заморожена после 10-й (AC30)
- Содержит не более трёх вариантов (AC31)
- Ссылка открывается без cookies и без состояния сессии (AC32)
- Длина ссылки для трёх поездок остаётся в пределах, безопасных для мессенджеров
- Испорченная или несовместимая по версии ссылка → осмысленная страница, не 500 (AC33)
- Исчезнувшее предложение → замена с пометкой, а не пустая карточка
- Пересборка по одной и той же ссылке даёт те же направления
...
- Испорченное состояние сессии в браузере → чистый лист, а не упавший экран
- Полная недоступность MCP → событие `unavailable`, а не пустой `done` (AC10a)
...
|---|---|---|
| Живые данные догоняют снапшот слишком долго | Пользователь видит устаревшие цены дольше допустимого | Замеры на U1; параллельные транспорт и жильё (KTD8), потоковый контракт в U4, замер на production в U6 |

 ...
 *
 * Тридцать, а не двенадцать: ответ отсортирован по цене, и на направлениях,
 * где самолёт дороже поезда, он в первую дюжину не попадает. Замер 13 августа,
 * Москва → Сочи: при page_size 12 приходят только поезда и автобусы, при 30
 * добавляются 15 авиавариантов. Из-за этого дизлайк «долго добираться» не мог
 * поднять быструю дорогу — её просто не было в пуле. На время ответа это не
 * влияет (замер: 4 547 мс против 2 096 мс, разница в пределах шума сети),
 * объём ответа растёт с 36 до 67 КБ.
 */
